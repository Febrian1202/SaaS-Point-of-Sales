...
import type {
  ArgsBrilink,
  ArgsGetBrilink,
  ArgsGetBrilinkDetail,
  ArgsGetSummaryBrilink,
} from "./schema";
...
export const createBrilinkTransaction = async (
  tenantId: string,
  cashierId: string,
  body: ArgsBrilink,
) => {
  const existedTransaction = await db.query.brilinkTransactions.findFirst({
    where: and(
      ...
      eq(brilinkTransactions.referenceNumber, body.referenceNumber),
    ),
  });
...
export const getBrilinkTransaction = async (
  tenantId: string,
...
export const getBrilinkTransactionDetail = async (
  tenantId: string,
  param: ArgsGetBrilinkDetail,
) => {
  const { id } = param;
