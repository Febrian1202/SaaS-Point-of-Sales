...
const baseBrilinkSchema = createInsertSchema(brilinkTransactions, {
  trxType: trxType,
  agentCommission: t.Numeric({
    exclusiveMinimum: 0,
    error: validationDetail("Agent commission cannot empty!"),
  }),
  customerAmount: t.Numeric({
    minimum: 0,
    error: validationDetail("Customer amount invalid!"),
  }),
  adminFeeCharged: t.Numeric({
    minimum: 0,
    error: validationDetail("Admin fee invalid!"),
  }),
  notes: t.Optional(
    t.String({
      pattern: REGEX_SAFE_NOTES,
      error: validationDetail(
        "Notes must not contain the characters <, >, {, or }.",
      ),
    }),
  ),
  referenceNumber: t.String({
    ...
    error: validationDetail(
      "The reference number may only contain letters and numbers.",
    ),
  }),
});

export const schemaBodyBrilink = t.Omit(baseBrilinkSchema, [
  "id",
  "status",
  "tenantId",
  "createdAt",
...
export const schemaParamsDetailBrilink = t.Object({
  id: t.String({
    format: "uuid",
    error: validationDetail("ID must be in UUID"),
  }),
});
