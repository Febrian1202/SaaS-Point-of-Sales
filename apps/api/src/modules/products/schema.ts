...
export const schemaQueryProduct = t.Object({
  ...
  barcode: t.Optional(
    t.String({ error: validationDetail("Barcode must be a string") }),
  ),
    ...
    t.Union(
      ...
      ],
      {
        error: validationDetail(
          "Status must be AVAILABLE, LOW_STOCK, or OUT_OF_STOCK",
        ),
      },
    ),
  ),
  ...
  limit: t.Optional(
    t.Numeric({
      default: 10,
      error: validationDetail("Limit must be a number"),
    }),
  ),
});
...
const unfilteredBodyProduct = createInsertSchema(products, {
  barcode: t.Optional(
    t.String({
      ...
      error: validationDetail(
        "The barcode may only contain letters, numbers, hyphens, and underscores.",
      ),
    }),
  ),
  name: t.String({
    ...
    error: validationDetail(
      "The product name may only contain letters, numbers, and basic symbols.",
    ),
  }),
});

export const schemaBodyProduct = t.Omit(unfilteredBodyProduct, [
  "id",
  "createdAt",
  "updatedAt",
  "isActive",
...
export const schemaResponseGetDetail = withSuccess(productPlusCategory);
...
export const schemaResponsePatch = withSuccess(baseProduct);
