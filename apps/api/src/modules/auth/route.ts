...
import { registerBusiness, updateRefreshToken, verifyUsers } from "./service";
...
export const authRoutes = new Elysia({
  ...
  .post(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Endpoint ini digunakan untuk memvalidasi email dan password. Jika berhasil, sistem akan mengembalikan `accessToken` dan otomatis memasang `refreshToken` ke dalam **HttpOnly Cookie**.",
      },
    },
  )
  .post(
    "/register",
    async ({ body, set, accessJwt, refreshJwt, cookie: { refreshToken } }) => {
      const result = await registerBusiness(body);

      if (!result.user || !result.store)
        throw new RegisterError(
          "Failed when creating account, please try again!",
        );
  ...
  .post(
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Mengeluarkan `accessToken` baru ketika token lama sudah kedaluwarsa. Endpoint ini membaca `refreshToken` secara otomatis dari **HttpOnly Cookie** klien. Mekanisme ini memastikan sesi operasional kasir tetap berjalan lancar tanpa perlu login ulang berulang kali.",
      },
    },
  )
  .use(authPlugin)
  .post(
    "/logout",
    async ({ userId, cookie: { refreshToken } }) => {
    ...
    },
    {
      ...
      detail: {
        ...
        description:
          "Mengakhiri sesi pengguna secara permanen. Endpoint ini akan menghapus jejak `refreshToken` yang ada di database demi keamanan, sekaligus memberikan instruksi kepada browser untuk memusnahkan cookie sesi pengguna tersebut.",
      },
    },
  )
  .use(adminGuard);
