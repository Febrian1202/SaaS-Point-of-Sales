<script lang="ts">
	...
	// Tentukan rute aktif secara reaktif berdasarkan URL
	...
	const allowedMenuItems = $derived(
		menuItems.filter((item) => item.roles.includes(user?.role || 'cashier'))
	);
</script>
...
>
	...
	<div class="mb-8 px-2">
		...
		<p class="mt-1 font-mono text-[10px] tracking-wider text-muted-foreground uppercase">
			POS System
		</p>
	</div>
	...
	<nav class="flex-1 space-y-1.5">
		{#each allowedMenuItems as item (item.path)}
